...
- [ ] **Step 3b: Guard the promise that a rejected send costs nothing**

Append to `web/src/app/app.spec.ts`. This is the one behaviour `App` genuinely owns rather
than delegates: the guarantee lives in the *ordering* of `submit()`'s try/catch, so a
refactor that moved `store.finish` above the `await` would break the promise with every
